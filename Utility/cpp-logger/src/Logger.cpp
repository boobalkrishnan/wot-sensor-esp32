...
string Logger::getCurrentTime()
{
   ...
   auto ms = std::chrono::duration_cast<std::chrono::microseconds>(hnow.time_since_epoch()) -
            //  std::chrono::duration_cast<std::chrono::milliseconds>(hnow.time_since_epoch()) -
             (std::chrono::duration_cast<std::chrono::seconds>(hnow.time_since_epoch()));
   const std::time_t t_c = std::chrono::system_clock::to_time_t(hnow);
   // Convert current time to string
   currTime.assign(ctime(&t_c));
   string currentTime = currTime.substr(0, currTime.size()-1);
   currentTime.append (" ");
   currentTime.append (std::to_string(ms.count()));
...
void Logger::PrintLog(Logging::LogLevel loglevel, const char* Component, const char* LogText, const char* LogText1) //Logging::LogLevel loglevel
{
    std::ostringstream LogData;
	LogData.flags(std::ios::hex);
    LogData << Component<<":"<<LogText<<" "<<LogText1;
    debug(LogData);
}


void Logger::PrintLog(Logging::LogLevel loglevel, const char* Component, const char* LogText, int data) //Logging::LogLevel loglevel
{
    std::ostringstream LogData;
	LogData.flags(std::ios::hex);
    LogData << Component<<":"<<LogText<<" "<<data;
    debug(LogData);
}

void Logger::PrintLog(Logging::LogLevel loglevel, const char* Component, const char* LogText) //Logging::LogLevel loglevel
{
    std::ostringstream LogData;
    LogData << Component<<":"<<LogText;
    debug(LogData);
}

void Logger::PrintLog(Logging::LogLevel loglevel,  const char* Component, int data) //Logging::LogLevel loglevel
{
    std::ostringstream LogData;
	//LogData.flags(std::ios::hex);
    LogData << Component<<":"<<data;
    debug(LogData);
}